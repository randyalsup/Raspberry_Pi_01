...
// Purpose: UART (Pi 5) <-> ESP-NOW (Remote ESP32) Bridge
// ========================================

// Remote ESP32 MAC address
uint8_t remoteMAC[] = {0x80, 0xF3, 0xDA, 0x42, 0xCC, 0xA0}; // RC device
// This device MAC: EC:E3:34:22:D9:B0
...
void forwardToRemote(UARTMessage *msg);
...
void processUARTFromPi() {
  ...
  while (Serial2.available()) {
    ...
    switch(state) {
      ...
      case 5:
        if (b == END_BYTE) forwardToRemote(&msg);
        state = 0;
        break;
    }
  }
...
void forwardToRemote(UARTMessage *msg) {
  ...
  esp_now_send(remoteMAC, packet, 2 + msg->length);
}
...
void setup() {
  ...
  if (esp_now_add_peer(&peerInfo) == ESP_OK) {
    remotePaired = true;
    Serial.println("Paired with remote");
  }
...
void loop() {
  ...
  // TODO: UWB ranging when modules arrive
